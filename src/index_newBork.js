...
async function updateFromTelegram(extensionAPI) {
  ...
  let corsProxyUrl =
    stripTrailingSlash(
      unlinkify(
        extensionAPI.settings.get('proxy')
      ))

  let inboxName = extensionAPI.settings.get('inbox-name');
  ...
  let updateResponse = await GET(`getUpdates?offset=0&timeout=60`)
  ...
  let maxOrder = findMaxOrder(inboxUid)
    // inbox location
  let inboxLocation  = extensionAPI.settings.get('inbox-name') || 0;
  ...
  if (updateResponse.result.length) {
    ...
    extensionAPI.settings.set('update-id', lastUpdate.update_id)
  ...
  async function handleTelegramUpdate(result, i) {
    ...
    async function handleMessage() {
      ...
      let timestampLocation  = extensionAPI.settings.get('timestamp-location') || "NONE" ;

      let senderLocation = extensionAPI.settings.get('sender-location') || "NONE" ;
      ...
      try {
        let result = await runWithMutualExclusionLock({
          ...
          action: async () => {
            ...
            return await updateFromTelegram(extensionAPI)
          }
        })
...
async function onload({extensionAPI}) {
  ...
  // stock telegram stuff here
  let telegramApiKey = extensionAPI.settings.get('api-key');

  startTelegroam(extensionAPI)
}
